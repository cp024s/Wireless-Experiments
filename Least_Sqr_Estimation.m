...
close all;
%%%%% Assumptions%%%%%
Nfft=32; %no. of FFT subcarriers
Ng=Nfft/8; %no. of CP symbols
Nofdm=Nfft+Ng; %total length of one OFDM symbol

Nps=4; % spacing between the pilot subcarriers
Np=Nfft/Nps;% No. of pilot symbols per OFDM symbol
Nm=Nfft-Np; % No. of message symbols (unknown symbols)
...
%QAM Modulation for message symbols
Nbps= 4; %QAM modulation
M=2^Nbps; % no. of possible QAM synbols
Es=1;
A= sqrt(3/2/(M-1)*Es); %normalaization factor
Data= A*qammod(msgint,M);
% Loading both pilot and message symbols on to subcarriers
X=[]; % New OFDM symbol
l=1; ll=1;
for k=1:32
if mod(k,4)==1
...
% Generating channel
h=(1/2)*[randn+j*randn randn+j*randn]; % Random rayleigh
channel of two taps
% Received Signal
y=conv(x_cp,h);
...
% Estimating the Sub-channel information at the pilot location
using LS
% method
% Finidng the pilot locations
pilot_loc=[];
for k=1:Nfft
if(mod(k,Nps)==1)
...
H_LS=Y_rx(pilot_loc)./Xp; % LS estimation of subchannels at
pilot location

% Interpolation, to estimate the sub-channel gains in
remaining
% subchannels

slope = (H_LS(end)-H_LS(end-1))/(pilot_loc(end)-pilot_loc(end-
1));
...
H_interpolated_power_dB=10*log(abs(H_interpolated.*conj(H_inte
rpolated)));
plot(H_power_dB,'b');
hold on;
